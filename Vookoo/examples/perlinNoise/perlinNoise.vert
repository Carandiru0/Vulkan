...
layout (set = 0,binding = 0) uniform Uniform {
  ...
  mat4 view;
  mat4 world;
  float iTime[4]; // shader playback frame
} u;
...
layout (location = 5) in float instanceT0;

#define time (u.iTime[0]*0.1-instanceT0)

// Copy noise to the fragment shader.
...
void main() {
  ...
  // vertex shader works best with object having mean radius of 20. aka mean_radius(position)=20.
  ...
  gl_Position = u.projection * u.view * u.world * vec4(instanceRotation() * instanceScale * newPosition + instancePos, 1.0);
