...
// for proper scaling of dpi at the correct granularity of 8
static __inline int const roundToNearestMultipleOf(int const n, int const multiple)
{
    int const half = (multiple >> 1);
    ...
    switch (uMsg)
    {
        ...
        case WM_SYSCOMMAND:
        {
            switch (wParam & 0xfff0)
            {
                ...
                {
                    if (window->monitor || _glfwPlatformWindowFocused(window))
                    {
                        // We are running in full screen mode -or- window is currently active 
						// so disallow
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    if (!window->monitor)
    {
        ...
        // bugfix: always force a granularity of 8 pixels for frameBuffer!
        // ensure the granularity of the used framebuffer resolution is still a multiple of 8
        // important for fastest transfer speed on transfer queue //
        #define granularity 8

        rect.right = roundToNearestMultipleOf((int)(rect.right), granularity);
        rect.bottom = roundToNearestMultipleOf((int)(rect.bottom), granularity);
